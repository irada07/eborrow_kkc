...
class ReportController extends Controller
{
    ...
    public function showGoodReport(Request $req){
        ...
        if($select == 1){
            $goodsub = Good::where('good_no','like','%(%')->pluck('good_no')->toArray();
            $goods = Good::with('unit', 'department')->whereNotIn('good_no',$goodsub)
                ->orderBy('updated_at', 'desc')
                ->get();
                foreach($goods as $key => $value){
                    $good_sub = Good::where('good_no','like','%'.$value['good_no'].'%')
                                    ->whereNotIn('good_no',[$value['good_no']])
                                    ->orderBy('good_no','ASC')->get();
                    $text_sub = "";
                    if(count($good_sub) > 1){
                        foreach($good_sub as $key_sub => $value_sub){
                            $text_sub = $text_sub."<p>".($key_sub+1).". ".$value_sub['good_no']." : ".$value_sub['name'];
                            if($value_sub['ready_to_use'] != 0){
                                $text_sub = $text_sub.' [ปกติ]';
                            }else{
                                $text_sub = $text_sub.' [ชำรุด]';
                            }
                        }
                        $goods[$key]['name_ex'] = 'ประกอบด้วย'.$text_sub;
                    }
                }
            ...
            $goods = Good::with('unit', 'department')
                ->where('price_unit' , '<', 5000)->whereNotIn('good_no',$goodsub)
                ->orderBy('updated_at', 'desc')
                ->get();
                foreach($goods as $key => $value){
                    $good_sub = Good::where('good_no','like','%'.$value['good_no'].'%')
