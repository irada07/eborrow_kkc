    ...
    */
    ...
    public function getLogin(Request $req)
    {
        ...
        if ($attribs['title'] == 'Students' ){
            if($attribs['programId'] != "3333012" && $attribs['programId'] != "3333013"){
                return redirect(route('login'))->withError('ผู้ใช้งานไม่ตรงตามเงื่อนไขที่กำหนด loop1');
            }
        }else {
            if ($attribs['campus'] != "Khonkean") {
                return redirect(route('login'))->withError('ผู้ใช้งานไม่ตรงตามเงื่อนไขที่กำหนด loop2');
            }else{
                if ($attribs['department'] != "สาขาวิศวกรรมคอมพิวเตอร์") {
                    return redirect(route('login'))->withError('ผู้ใช้งานไม่ตรงตามเงื่อนไขที่กำหนด loop3');
                }
            }
        }
